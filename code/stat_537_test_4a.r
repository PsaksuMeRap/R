...
aov.full <- aov(glm(y~ x1 + x2 + x3), type = "1")
aov.full
aov.1 <- aov(glm(y~ x2), type = "1")
aov.1
aov.2 <- aov(glm(y~ x1 + x2), type = "1")
aov.2
aov.3 <- aov(glm(y ~ x1), type = "1")
aov.3
aov.4 <- aov(glm(y~ x3), type = "1")
aov.4
...
#c.) refer to the scan --add the covariance matrix (4*cov(b1,b2))
